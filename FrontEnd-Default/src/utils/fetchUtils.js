...
async function deleteAndTransferItem(url, id, newId) {
  try {
    ...
    if (!res.ok) {
      console.error(`Failed to delete and transfer item with ID ${id} to ${newId}: ${res.status}`);
      return res.status;
    }
    return res.status;
  } catch (error) {
...
export { getItems, getItemById, deleteItemById, addItem, editItem , deleteAndTransferItem}
